...
## ⚠️ Dev Agent 禁止重写警告（2026-05-03 新增）

**教训**：2026-05-03，V17 迭代委托时 dev agent 完全重写了游戏（5127行→1018行），游戏从完整的 DBG 系统退化为只有 5个敌人+10张卡的简化版。已回滚代码重新委托。

**规则**：在委托 dev agent 前后必须执行以下检查：

### 委托前 - 在 prompt 中明确警告
```
禁止重写游戏！当前 index.html 有 5000+ 行，是完整的 DBG 卡牌游戏。
你只能修改现有代码，不能重写。修改后文件必须仍然有 5000+ 行。
```

### 交付后 - 立即验证文件完整性
```bash
wc -l index.html  # 必须仍然是 5000+ 行
grep -c "CARD_REWARDS\|gameState\|showDeckPreview" index.html  # 关键函数必须存在
```

### 如果被重写 - 立即回滚
```bash
cd /mnt/c/Users/YeZhimin/Desktop/card-game-prototype
git checkout cb9a487 -- index.html  # 回滚到上一个已知 good commit
```
