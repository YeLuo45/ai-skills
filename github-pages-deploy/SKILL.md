...
### Pattern B: peaceiris/actions-gh-pages (Recommended when legacy Pages is already configured)

Best when: GitHub Pages is already set to serve from the `gh-pages` branch (legacy mode), and you want CI/CD from the main project. This avoids interference between `actions/deploy-pages` and legacy Pages mode.
...
on:
  ...
  workflow_dispatch:
permissions:
  contents: write        # REQUIRED: must be 'write', not 'read'
  pages: write
  id-token: write
concurrency:
  group: pages
...
1. Set Pages source to `gh-pages` branch (not "GitHub Actions"):
   ```bash
...
**Why this pattern works when `actions/deploy-pages` fails:**
- `actions/deploy-pages` + `actions/upload-pages-artifact` uses GitHub's internal Pages deployment API
- If Pages is already configured for legacy mode (gh-pages branch), the internal API conflicts
- `peaceiris/actions-gh-pages` bypasses the internal API entirely — it pushes files to the `gh-pages` git branch like a normal git push, which legacy Pages serves directly
- Legacy Pages watching the `gh-pages` branch sees the new commit and rebuilds automatically
