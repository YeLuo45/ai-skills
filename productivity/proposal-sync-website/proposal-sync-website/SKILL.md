...
# 1. 获取当前 SHA
SHA=$(gh api repos/YeLuo45/prj-proposal-management/contents/data/proposals.json --jq '.sha')
...
# 获取当前 SHA
url = "https://api.github.com/repos/YeLuo45/prj-proposal-management/contents/data/proposals.json"
...
curl -s https://api.github.com/repos/YeLuo45/prj-proposal-management/contents/data/proposals.json

# 或用 gh api（不会 rate limit）
gh api repos/YeLuo45/prj-proposal-management/contents/data/proposals.json --jq '.content' | base64 -d | python3 -m json.tool | head -30

# 打开网站验证（需输入 GitHub Token）
open https://yeluo45.github.io/prj-proposal-management/
```

### ⚠️ 双层 Bug 诊断模式（2026-05-03 教训）

当网站显示数据异常（如"只有3个项目"或"格式错误"）时，需要同时排查**两层**：

**第一层：前端解析**
- `App.jsx` 的 `loadProposals()` 检查：`data.projects`（v2）还是 `data.proposals`（v1）
- v2 数据结构：`{version: 2, projects: [{proposals: [...]}]}`
- v1 数据结构：`{proposals: [...]}`
- 如 GitHub 是 v2 但前端只支持 v1 → 修复前端 `loadProposals()`

**第二层：sync 脚本配置**
- 检查 `REPO_NAME` 是否与实际仓库名一致
- **当前网站仓库：`prj-proposal-management`**（不是 `proposals-manager`）
- 如脚本配置了错误仓库名 → 79个提案从未同步到正确位置
- 验证方法：对比 `curl raw.githubusercontent.com/{owner}/{REPO_NAME}/main/data/proposals.json` 里的提案数量与 `~/.hermes/proposals/proposal-index.md` 里的提案数量

**诊断流程：**
```
网站显示异常 → 检查 GitHub raw JSON 实际数据格式（v1还是v2）→ 
检查前端是否支持该格式 → 
如数据量少 → 检查 sync 脚本 REPO_NAME 是否匹配实际仓库 → 
修复后重新 sync
```

**本次教训：**
- sync 脚本 REPO_NAME 配置为 `proposals-manager`，但网站仓库是 `prj-proposal-management`
- 导致79个提案从未同步到正确位置
- 同时前端 App.jsx 只支持 v1 格式，不支持 v2 嵌套格式
- 两处同时修复后才解决问题
