...
6. **提交前 `git status` 确认只有目标文件，避免混入无关文件**
7. **dual-branch 模式下：master 管源码，gh-pages 只管部署，不要在 gh-pages 上做开发**

---

## GitHub Pages Actions: 409 Conflict on deploy-pages

**症状：** `actions/deploy-pages@v4` 步骤持续失败（HTTP 409 Conflict），但 build 步骤成功。

**根因：** GitHub Pages 正在重建时（上一个 workflow 或内置 rebuild 还在运行），新的 `deploy-pages` 又来了。Pages 同时只允许一个 active deployment。
...
req = urllib.request.Request("https://api.github.com/repos/OWNER/REPO/pages",
    headers={"Authorization": "token TOKEN"})
pages = json.loads(urllib.request.urlopen(req, context=ctx).read())
print(pages["status"])  # "building" = 正在重建，"built" = 完成
```

**修复（按可靠性排序）：**

1. **等 Pages 完成后再触发 workflow** — 先查 status，只有 "built" 或 "errored" 时才 dispatch。

2. **workflow 加上 `cancel-in-progress: true`** — 取消排队的旧 run，只保留最新：
   ```yaml
   concurrency:
     group: "pages"
...
3. **通过 API trigger 而非 push commit** — 更好地控制时机：
   ```python
...
## WSL 网络：urllib SSL EOF 错误

**症状：** Python `urllib.request` 调用 GitHub API 时报 `ssl.SSLEOFError` 或 handshake 超时，但终端里 `gh` CLI 正常。

**根因：** WSL 网络栈对 GitHub 的 SSL 连接不稳定。
